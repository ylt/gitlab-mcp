...
class GraphQLError(BaseModel):
    ...
    extensions: dict[str, Any] | None = Field(default=None, description="Error extensions")


class GraphQLResponse(BaseGitLabModel):
    """GraphQL query response wrapper."""

    data: dict[str, Any] | None = Field(default=None, description="Query result data")
    errors: list[GraphQLError] | None = Field(default=None, description="Query errors if any")
...
class PageInfo(BaseGitLabModel):
    ...
    has_previous_page: bool = Field(default=False, description="Whether there are previous pages")
...
class PaginationResult(BaseGitLabModel):
    ...
    all_pages: list[dict[str, Any]] = Field(default_factory=list, description="All fetched pages")
    ...
    errors: list[GraphQLError] | None = Field(default=None, description="Any errors encountered")
    pages_fetched: int | None = Field(default=None, description="Pages fetched before error")
